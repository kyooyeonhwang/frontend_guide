  ...
  $.when($.get(csv_path)).then(
    function(data){      
      ...
      for (head_id = 0; head_id < csv_data[0].length; head_id++) { 
        table_head += "<th class=" + csv_data[0][head_id] +  ">" + csv_data[1][head_id] + "</th>";
      }
      ...
      for (row_id = 2; row_id < csv_data.length; row_id++) { 
        var row_html = "<tr>";

          for (col_id = 0; col_id < csv_data[row_id].length; col_id++) { 
            row_html += "<td class=" + csv_data[0][col_id] + ">" + csv_data[row_id][col_id] + "</td>";
          }
